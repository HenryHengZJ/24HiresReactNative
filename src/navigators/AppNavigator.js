...
const AppRouteConfigs1 = createStackNavigator(
  {
    Main: { 
        screen: Main,
        navigationOptions: {
...
class ReduxNavigation extends PureComponent {
  ...
  handleHardwareBack = () => { 
   ...
   //alert(JSON.stringify(state))
    ...
    if ((state.index === 0 && currentRouteName === "LoggedIn") || currentRouteName === "LoggedOut") {
    ...
    /*if (currentRouteName === "EditProfile")  {
      Alert.alert(
        'Save Draft',
        ...
        [
          {text: 'Back without Saving',
            onPress: () => 
              dispatch(NavigationActions.back()), style: 'cancel'
            },
          {text: 'Save', 
            onPress: () => 
    ...
    if (currentRouteName === "FirstLogin")  {
      //Prevent go Back
    }
