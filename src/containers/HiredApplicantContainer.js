...
class HiredApplicantContainer extends Component {
  ...
  fetchUserData = () => {
   ...
   // var url = apis.GETHiredApplicants_BASEURL+'limit='+loadlimit+'&postkey='+postkey+'&userid='+userid;
    var url = apis.GETApplicant_BASEURL+'limit='+loadlimit+'&postkey='+postkey+'&userid='+userid + '&status=hired';

    if (lasttime) {
        url = url + '&time__lt=' + lasttime;
    }

    console.log('HiredApplicantContainer url = ' + url + '\n' + 'userid= ' + userid)
    ...
    .then((response) => {
        ...
        this.setState({
            ...
            totalhiredapplicants: totalcount,
        },() => {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if ( nextProps.getHiredApplicant.hiredapplicant ) {

      if (this.props.getHiredApplicant.hiredapplicant !== nextProps.getHiredApplicant.hiredapplicant){
        if (nextProps.getHiredApplicant.hiredapplicant.status === 'hired') {
          this.handleRefresh()
        }
      }
    }
  ...
  renderHeader = () => {
    ...
    return (
      <View style={{flexDirection: 'row'}}>
        <Text style={[styles.heading, {color: colors.green02}]}>{this.state.totalhiredapplicants}</Text>
        <Text style={[styles.heading, {color: colors.greyBlack }]}>Hired Applicants</Text>
      </View>
    )
    ...
    return (
        <View>
            <ScrollView style={styles.scrollView}>
                ...
                <View style={{flexDirection: 'row'}}>
                  <Text style={[styles.heading, {color: colors.green02}]}>0</Text>
                  <Text style={[styles.heading, {color: colors.greyBlack }]}>Hired Applicants</Text>
                </View>
  ...
  renderFlatListView() {
    return (
        <List  containerStyle={{ marginTop:0, paddingTop:0, paddingHorizontal: 5, borderBottomWidth: 0, borderTopWidth: 0 }}>
           ...
           <FlatList
              extraData={this.state.totalhiredapplicants}
...
HiredApplicantContainer.propTypes = {
...
const mapStateToProps = state => ({
  ...
  getHiredApplicant: state.getHiredApplicant,
});

export default connect(mapStateToProps)(HiredApplicantContainer);
